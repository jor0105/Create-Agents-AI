...
from createagents.domain import ChatResponse, ToolCallInfo
...
class TestChatResponse:
    ...
    def test_to_dict_with_multiple_tool_calls(self):
        ...
        response = ChatResponse(
            content='Search completed', tool_calls=tool_calls
        )
...
class TestChatResponseEdgeCases:
    ...
    def test_multiple_responses_are_independent(self):
        ...
        assert (
            response1.tool_calls[0].tool_name
            != response2.tool_calls[0].tool_name
        )
    ...
    def test_large_number_of_tool_calls(self):
        tool_calls = [
            ToolCallInfo(f'tool_{i}', {'index': i}, f'result_{i}')
            for i in range(100)
        ]
