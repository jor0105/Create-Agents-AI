...
class RecursiveCharacterTextSplitter:
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        chunks = self._split_recursive(
            text, self.separators.copy(), self.chunk_size, self.chunk_overlap, depth=0
        )
    ...
    def _split_recursive(
        ...
        if depth >= self.max_recursion_depth:
            logger.warning(
                f"⚠️  Limite de recursão atingido (depth={depth}). Forçando divisão simples."
            )
                ...
                if self.length_function(chunk) > chunk_size:
                    ...
                    final_chunks.extend(
                        self._split_recursive(
                            chunk, separators[1:], chunk_size, chunk_overlap, depth + 1
                        )
                    )
                else:
...
class EmbeddingGenerator:
        ...
        try:
            ...
            if self.normalize:
                if not self._is_normalized(embedding):
                    logger.debug(
                        f"Embedding não normalizado pelo modelo {self.model_name}"
                    )
    ...
    def close(self):
        ...
        if stats["embeddings_generated"] > 0:
            norm_percent = (
                stats["normalizations_applied"] / stats["embeddings_generated"]
            ) * 100
            logger.info(
...
class VectorStore:
    ...
    def add_batch(
        ...
        if not self._is_open:
            raise RuntimeError(
                "VectorStore não está aberto. Use 'open()' ou context manager."
            )
        ...
        if deduplicate:
            ...
            if self.duplicates_skipped % 100 == 0 and self.duplicates_skipped > 0:
                logger.debug(
                    f"🗑️  Duplicados removidos até agora: {self.duplicates_skipped}"
                )
    ...
    def save_and_close(self):
        ...
        try:
            if self.index is not None and self.total_chunks > 0:
                ...
                logger.info(
                    f"✅ {self.total_chunks} chunks salvos em {self.faiss_path}"
                )
...
def create_embeddings(
    ...
    try:
        _validate_inputs(
            documents, model_name, chunk_size, chunk_overlap, batch_size, num_workers
        )
    ...
    try:
        ...
        for doc_path in documents:
            ...
            try:
                ...
                # Se for arquivo, processa diretamente
                    ...
                    try:
                        chunks = list(
                            loader.load(path, custom_metadata=custom_metadata)
                        )
                        all_chunks.extend(chunks)
                        if chunks:
                            files_processed += 1
        ...
        with VectorStore(
            ...
            for i in range(0, len(all_chunks), batch_size):
                ...
                logger.info(
                    f"⚙️  Lote {batch_num}/{total_batches} ({len(batch)} chunks)"
                )
                ...
                if valid_embeddings:
                    store.add_batch(
                        valid_chunks, valid_embeddings, deduplicate=deduplicate
                    )
        ...
        return {
            ...
            "chunks_processed": (
                chunks_processed if "chunks_processed" in locals() else 0
            ),
            ...
            "embeddings_stats": (
                ...
                else {"embeddings_generated": 0, "normalizations_applied": 0}
            ),
            "config": {
                "model": model_name,
                "chunk_size": chunk_size,
    ...
    try:
        with open(f"{output_prefix}_stats.json", "r", encoding="utf-8") as f:
            ...
            result["total_chunks"] = saved_stats.get(
    ...
    logger.info(
        f"📁 Arquivos: {result['files_processed']} processados | {result['files_failed']} falharam"
    )
    logger.info(
        f"🔧 Índice: {result['index_type']} {'(automático)' if result['index_auto_selected'] else '(manual)'}"
    )
...
if __name__ == "__main__":
    ...
    result = create_embeddings(
        documents=[
            "/home/jordan/Downloads/Databases/dados_bolsa_br/Docs_Cvm/DFP/2024/dfp_cia_aberta_DRE_con_2024.parquet"
        ],
        model_name="qwen3-embedding:4b",
        chunk_size=1000,
        chunk_overlap=150,
    ...
    if result.get("total_chunks", 0) > 0:
        ...
        print("\n📊 Detalhes técnicos:")
        print(
            f"  - Dimensão dos embeddings: {result.get('embedding_dimension', 'N/A')}"
        )
