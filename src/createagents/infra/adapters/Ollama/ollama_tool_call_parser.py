...
class OllamaToolCallParser:
    ...
    def has_tool_calls(response: str) -> bool:
        ...
        has_calls = bool(
            OllamaToolCallParser.TOOL_CALL_PATTERN.search(response)
        )
        if has_calls:
            OllamaToolCallParser._logger.debug(
    ...
    def extract_tool_calls(response: str) -> List[Dict[str, Any]]:
        ...
        OllamaToolCallParser._logger.debug(
            'Extracting tool calls from Ollama response'
        )
        tool_calls = []
    ...
    def _parse_xml_tool_call(xml_content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for child in args_elem:
                ...
                arguments[arg_name] = OllamaToolCallParser._convert_value(
                    arg_value
                )
    ...
    def remove_tool_calls_from_response(response: str) -> str:
        ...
        """
        cleaned = OllamaToolCallParser.TOOL_CALL_PATTERN.sub(
            '', response
        ).strip()
        if cleaned != response:
            OllamaToolCallParser._logger.debug(
                'Removed tool call tags from response'
            )
        return cleaned
